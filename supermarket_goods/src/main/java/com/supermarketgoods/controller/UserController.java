...
public class UserController {
    ...
    @PostMapping("/login")
    @ResponseBody
    public ServiceVO login(@RequestBody UserLogin userLogin, HttpSession session) throws Exception {
        String passed = userLogin.getPassword();
        MessageDigest messageDigest = MessageDigest.getInstance("SHA-256");
        byte[] hash = messageDigest.digest(passed.getBytes());
        String newpassed = bytesToHex(hash);
        userLogin.setPassword(newpassed);
    ...
    public ServiceVO updatePassword(String newPassword, HttpSession session) throws Exception {
        MessageDigest messageDigest = MessageDigest.getInstance("SHA-256");
        byte[] hash = messageDigest.digest(newPassword.getBytes());
        newPassword = bytesToHex(hash);
